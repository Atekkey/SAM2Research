...
def vos_separate_inference_per_object(
    ...
    if doBP: # Do imperfect BP after 5 frames
        delayTiming = 5
        # run inference separately for each object in the video
        object_ids = sorted(inputs_per_object)
        output_scores_per_object = defaultdict(dict)
        for object_id in object_ids:
            ...
            for out_frame_idx, _, out_mask_logits in predictor.propagate_in_video(
                ...
                if(curMaskFound and (prevMaskLost or curDelay > 1) ): # Found. First or again
                    if(curDelay < delayTiming): # Delay hasnt hit yet
                        curDelay += 1
                    else: # Delay hit
                        ...
                        curDelay = 1
                if( (not curMaskFound) and curDelay > 1 ): # Lost but should have BP'd
                    print("BP almost cut off, frame ", out_frame_idx)
                    inference_stateBP = inference_state.copy() # DUPLICATE THE STATE
                    numFrames = (out_frame_idx - 1) - max(lostIdx , min(input_frame_inds))    # MIND THE -1
                    print("Begin BackProp for ", numFrames, " frames. Starting at ", out_frame_idx - 1)
                    numberOfBPs += 1

                    for inner_frame_idx, _2, inner_out_mask_logits in predictor.propagate_in_video( # BP LOOP
                    inference_stateBP, start_frame_idx=out_frame_idx - 1, 
                    max_frame_num_to_track = numFrames  , reverse=True,):
                        obj_scores2 = inner_out_mask_logits.cpu().numpy()
                        output_scores_per_object[object_id][inner_frame_idx] = obj_scores2
                    ...
                    lostIdx = out_frame_idx + 1
                    curDelay = 1
                elif((not curMaskFound) and (not prevMaskLost) ): # Lost for first time
                    prevMaskLost = True
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False, # AJT
        default = "/work/hdd/bdnb/atekkey/sam2/notebooks/results/bp_v1",
