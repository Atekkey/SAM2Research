...
def vos_separate_inference_per_object(
    ...
    for object_id in object_ids:
        ...
        for out_frame_idx, _, out_mask_logits in predictor.propagate_in_video(
            ...
            if not maskExists(obj_scores) and not lost:         # First frame with no mask
                l = out_frame_idx
                lost = True
            if maskExists(obj_scores):
                lastFoundFrame = out_frame_idx
                if lost:            # First frame with a mask
                    r = out_frame_idx
                    frameTuples.append((l, r))
                    lost = False
        
        # BACK PROPAGATION START
        for i in range(len(frameTuples)-1, -1, -1):
            l, r = frameTuples[i]
            bestIndex = (r + lastFoundFrame)//2     # We select the midpoint frame in the following interval of found masks
            lastFoundFrame = l - 1
            #Add mask
            predictor2.reset_state(inference_state)
            thisMask = (torch.from_numpy(output_scores_per_object[object_id][bestIndex]) > score_thresh).cpu().numpy()[0][0]
            predictor2.add_new_mask( 
                inference_state=inference_state,
                frame_idx=bestIndex,
                obj_id=object_id,
                mask=thisMask,)

            for out_frame_idx, _, out_mask_logits in predictor2.propagate_in_video( # Bandaid backprop
                inference_state,
                start_frame_idx=bestIndex,
                max_frame_num_to_track = 1+bestIndex-l,
                reverse=True,
            ):
                obj_scores = out_mask_logits.cpu().numpy()
                union = (obj_scores > 0)  |  (output_scores_per_object[object_id][out_frame_idx] > 0) # union
                output_scores_per_object[object_id][out_frame_idx] = union
...
def main():
    ...
    ) #/work/hdd/bdnb/atekkey/sam2/sam2/configs/sam2.1/sam2.1_hiera_l.yaml
    parser.add_argument(
        "--sam2_checkpoint",
        type=str,
        default="/work/nvme/bdnb/atekkey/sam2/sam2_logs/configs/sam2.1_training/score_eff.yaml/checkpoints/checkpoint.pt",
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default = "/work/nvme/bdnb/atekkey/sam2/_results/BP_mem_enc_1",
