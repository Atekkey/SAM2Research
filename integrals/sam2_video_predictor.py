...
class SAM2VideoPredictor(SAM2Base):
    ...
    def add_new_points_or_box(
        ...
        current_out, _, my_iou = self._run_single_frame_inference(
    ...
    def propagate_in_video(
        ...
        for frame_idx in tqdm(processing_order, desc="propagate in video"):
            ...
            for obj_idx in range(batch_size):
                obj_output_dict = inference_state["output_dict_per_obj"][obj_idx]
                if "frame_usage" not in obj_output_dict:
                    # TODO: maybe bug for multiple objects where one appears later in the video
                    frame_usage = torch.zeros(len(processing_order), device=device)
                    frame_score = torch.zeros(len(processing_order), device=device)
                    obj_output_dict['frame_usage'] = frame_usage
                    obj_output_dict['frame_score'] = frame_score
                    obj_output_dict['start_frame_idx'] = start_frame_idx
                    obj_output_dict['past_iou'] = []
                ...
                if frame_idx in obj_output_dict["cond_frame_outputs"]:
                    ...
                    current_out, pred_masks, my_iou = self._run_single_frame_inference(
                        inference_state=inference_state,
            ...
            yield frame_idx, obj_ids, video_res_masks, my_iou
    ...
    def _run_single_frame_inference(
        ...
        current_out, my_iou = self.track_step(
            frame_idx=frame_idx,
            is_init_cond_frame=is_init_cond_frame,
        ...
        return compact_current_out, pred_masks_gpu, my_iou
