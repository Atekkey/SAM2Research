...
def vos_separate_inference_per_object(
    ...
    if doBP: # Do perfect BP with delay 5
        delayTiming = 5
        # run inference separately for each object in the video
        object_ids = sorted(inputs_per_object)
        output_scores_per_object = defaultdict(dict)
        for object_id in object_ids:
            ...
            for out_frame_idx, _, out_mask_logits in predictor.propagate_in_video(
                ...
                if(curMaskFound and (prevMaskLost or curDelay > 1)): # Found again
                    gtMask = None
                    gtMaskExists = False
                    try:
                        per_obj_input_mask, _ = load_masks_from_dir(input_mask_dir=input_mask_dir, 
                        video_name=video_name, frame_name=frame_names[out_frame_idx],
                        per_obj_png_file=per_obj_png_file, allow_missing=True, )
                        gtMask = per_obj_input_mask[object_id]
                        gtMaskExists = True
                    except Exception:
                        try:
                            inpMask, _ = load_masks_from_dir(input_mask_dir=input_mask_dir, 
                            video_name=video_name, frame_name=frame_names[out_frame_idx],
                            per_obj_png_file=False, allow_missing=False, )
                            gtMask = list(inpMask.items())[0][1]
                            gtMaskExists = True
                        except Exception:
                            pass
                        pass

                    if(curDelay < delayTiming and gtMaskExists): # Delay hasnt hit yet
                        curDelay += 1
                        continue
                    elif(gtMaskExists and (curDelay >= delayTiming)): # Delay has hit
                        # APPLY CURRENT GT MASK
                        inference_stateBP = inference_state.copy() # DUPLICATE THE STATE
                        predictor.add_new_mask( inference_state=inference_stateBP, frame_idx=out_frame_idx, obj_id=object_id, 
                        mask=gtMask,)
                        numberOfBPs += 1
                        numFrames = out_frame_idx - max(lostIdx , min(input_frame_inds))
                        print("Begin BackProp for ", numFrames, " frames. Starting at ", out_frame_idx)
                        for inner_frame_idx, _2, inner_out_mask_logits in predictor.propagate_in_video( # BP LOOP
                        inference_stateBP, start_frame_idx=out_frame_idx, 
                        max_frame_num_to_track = numFrames  , reverse=True,):
                            
                            obj_scores2 = inner_out_mask_logits.cpu().numpy()
                            output_scores_per_object[object_id][inner_frame_idx] = obj_scores2
                        
                        # Done reset vars
                        prevMaskLost = False
                        curMaskFound = True
                        lostIdx = out_frame_idx + 1
                        curDelay = 1

                if((not curMaskFound) and (not prevMaskLost) ): # Lost for first time
                    prevMaskLost = True
                    lostIdx = out_frame_idx
                    curDelay = 1
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False, ##
        default = "/work/hdd/bdnb/vpurushotham/datasets/LVOS/valid/JPEGImages", ##
    ...
    parser.add_argument(
        ...
        type=str,
        required=False, ##
        default = "/work/hdd/bdnb/vpurushotham/datasets/LVOS/valid/Annotations", ##
    ...
    parser.add_argument(
        ...
        type=str,
        required=False, # AJT
        default = "/work/hdd/bdnb/atekkey/sam2/notebooks/results/bp_ub2",
